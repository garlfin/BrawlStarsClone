...
{
    public Vector2D<float> MinTransformed =>
        new(Transform.Location.X - HalfLength.X, Transform.Location.Z - HalfLength.Y);

    public Vector2D<float> MaxTransformed =>
        new(Transform.Location.X + HalfLength.X, Transform.Location.Z + HalfLength.Y);

    public SquareCollider(Entity owner, bool isStatic = true, List<Entity>? ignoreList = null,
    ...
    protected override void ResolveX(Collision collision)
    {
        ...
        if (transform.X + HalfLength.X > otherTransform.X - HalfLength.X &&
            transform.X + HalfLength.X < otherTransform.X + HalfLength.X)
    ...
    protected override void ResolveY(Collision collision)
    {
        ...
        if (transform.Z + HalfLength.Y > otherTransform.Z - HalfLength.Y &&
            transform.Z + HalfLength.Y < otherTransform.Z + HalfLength.Y)
    ...
    {
        var t1 = (MinTransformed.X - ray.Position.X) / ray.Direction.X;
        var t2 = (MaxTransformed.X - ray.Position.X) / ray.Direction.X;
        var t3 = (MinTransformed.Y - ray.Position.Z) / ray.Direction.Z;
        var t4 = (MaxTransformed.Y - ray.Position.Z) / ray.Direction.Z;
