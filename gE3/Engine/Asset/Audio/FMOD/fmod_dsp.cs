...
public delegate Result
    DspGetspeakermodeFunc(ref DspState dspState, ref int speakermodeMixer, ref int speakermodeOutput);
...
public delegate Result DspDftFftrealFunc(ref DspState dspState, int size, IntPtr signal, IntPtr dft, IntPtr window,
    int signalhop);

public delegate Result DspDftIfftrealFunc(ref DspState dspState, int size, IntPtr dft, IntPtr signal, IntPtr window,
    int signalhop);

public delegate Result DspPanSummonomatrixFunc(ref DspState dspState, int sourceSpeakerMode, float lowFrequencyGain,
    float overallGain, IntPtr matrix);

public delegate Result DspPanSumstereomatrixFunc(ref DspState dspState, int sourceSpeakerMode, float pan,
    float lowFrequencyGain, float overallGain, int matrixHop, IntPtr matrix);

public delegate Result DspPanSumsurroundmatrixFunc(ref DspState dspState, int sourceSpeakerMode, int targetSpeakerMode,
    float direction, float extent, float rotation, float lowFrequencyGain, float overallGain, int matrixHop,
    IntPtr matrix, DspPanSurroundFlags flags);

public delegate Result DspPanSummonotosurroundmatrixFunc(ref DspState dspState, int targetSpeakerMode, float direction,
    float extent, float lowFrequencyGain, float overallGain, int matrixHop, IntPtr matrix);

public delegate Result DspPanSumstereotosurroundmatrixFunc(ref DspState dspState, int targetSpeakerMode,
    float direction, float extent, float rotation, float lowFrequencyGain, float overallGain, int matrixHop,
    IntPtr matrix);

public delegate Result DspPanGetrolloffgainFunc(ref DspState dspState, DspPan3DRolloffType rolloff, float distance,
    float mindistance, float maxdistance, out float gain);
...
{
    [FieldOffset(0)] public DspParameterDescFloat floatdesc;
    [FieldOffset(0)] public DspParameterDescInt intdesc;
    [FieldOffset(0)] public DspParameterDescBool booldesc;
    [FieldOffset(0)] public DspParameterDescData datadesc;
}
...
public struct DspParameterFft
{
    ...
    public void GetSpectrum(ref float[][] buffer)
    {
        ...
        for (var i = 0; i < bufferLength; ++i) GetSpectrum(i, ref buffer[i]);
    }
