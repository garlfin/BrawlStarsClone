...
public static class MeshLoader
{
    public static Mesh LoadMesh(string path)
    {
        ...
        for (int u = 0; u < meshCount; u++)
        {
            ...
            for (int i = 0; i < data.Vertices.Length; i++) data.Vertices[i] = reader.ReadVector3D();
            data.UVs = new Vector2D<float>[reader.ReadUInt32()];
            for (int i = 0; i < data.UVs.Length; i++) data.UVs[i] = reader.ReadVector2D();
            data.Normals = new Vector3D<float>[reader.ReadUInt32()];
            for (int i = 0; i < data.Normals.Length; i++) data.Normals[i] = reader.ReadVector3D();
            data.Faces = new Vector3D<int>[reader.ReadUInt32()];
            for (int i = 0; i < data.Faces.Length; i++) data.Faces[i] = reader.ReadVector3Di();
